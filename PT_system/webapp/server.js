...
const CANONICAL_EXERCISE_NAMES = {
  ...
  '머신 디클라인 펙덱 플라이 머신': '디클라인 펙덱 플라이 머신',
  ...
  '굿모닝 엑사사이즈 머신': '굿모닝 머신',
  '비하인드 펙덱 플라이 머신': '비하인드 펙덱 플라이 머신',
  '머신 비하인드 펙덱 플라이': '비하인드 펙덱 플라이 머신',
  '체스트 플라이 머신': '펙덱 플라이 머신',
};
...
function targetProfileAdjust(item) {
  ...
  if (/햄스트링\+둔근|둔근\+햄스트링|햄스트링.*둔근|둔근.*햄스트링/.test(target)) {
    return {
      role,
      purpose: ['후면 체인 연결 강화', '햄스트링 길이감과 둔근 수축 연결', '허리 보상 감소'],
      points: ['내려갈 때 햄스트링을 길게 만든다', '올라올 때 엉덩이로 바닥을 밀어낸다', '허리로 젖히지 않고 고관절만 접고 편다', '무릎 각도와 발 접지를 일정하게 유지'],
      keywords: ['허벅지 뒤 길어짐', '엉덩이 아래 묵직함', '둔근으로 마무리되는 느낌'],
      muscles: ['햄스트링', '대둔근'],
      signals: ['허리 과신전', '햄스트링만 쥐남', '둔근 수축 없이 올라오기'],
      next: '후면 체인 감각을 힙쓰러스트/런지/스쿼트 패턴으로 연결',
    };
  }
  ...
  if (/하체\s*전체/.test(target)) {
    return {
      role,
      purpose: ['하체 전체 협응', '발-무릎-골반 정렬 안정', '둔근과 대퇴사두의 힘 분배'],
      points: ['발바닥 전체를 균등하게 누른다', '무릎과 발끝 방향을 맞춘다', '골반이 한쪽으로 빠지지 않게 유지', '밀어낼 때 허리보다 하체 전체 압력으로 올라온다'],
      keywords: ['하체 전체 단단함', '발바닥 접지감', '무릎 흔들림 감소'],
      muscles: ['대퇴사두', '대둔근', '햄스트링', '내전근'],
      signals: ['무릎 안쪽 붕괴', '발 접지 풀림', '허리로 버티기'],
      next: '하체 전체 정렬 확인 후 스쿼트/런지/힌지 패턴으로 연결',
    };
  }
  ...
  if (/중부\s*승모|등\s*중앙/.test(target)) {
    return {
      role,
      purpose: ['견갑 후인 감각 강화', '등 중앙 수축 선명화', '팔 개입 감소'],
      points: ['팔로 당기기 전에 날개뼈를 먼저 모은다', '가슴을 패드나 자세 안에서 고정한다', '팔꿈치 경로를 뒤로 길게 유지', '목이 짧아지지 않게 어깨를 끌어올리지 않는다'],
      keywords: ['등 중앙 조여짐', '날개뼈가 붙는 느낌', '팔 힘이 빠지는 느낌'],
      muscles: ['중부 승모근', '능형근', '후면 삼각근 보조'],
      signals: ['팔로만 당김', '목/상부 승모 과긴장', '견갑이 먼저 움직이지 않음'],
      next: '견갑 후인 감각을 로우 계열 또는 후면 어깨 패턴으로 연결',
    };
  }

  if (/하부\s*승모/.test(target)) {
    return {
      role,
      purpose: ['견갑 하강 안정', '어깨 들림 억제', '오버헤드/당기기 전 견갑 위치 정리'],
      points: ['어깨를 귀에서 멀리 내린다', '날개뼈 아래쪽을 등 뒤 주머니로 넣는 느낌', '갈비가 들리지 않게 복압 유지', '팔보다 견갑 위치를 먼저 잡는다'],
      keywords: ['어깨가 내려앉는 느낌', '목 긴장 감소', '견갑 아래쪽 안정'],
      muscles: ['하부 승모근', '전거근 보조', '능형근 보조'],
      signals: ['어깨 으쓱', '갈비 들림', '허리 과신전'],
      next: '하부 승모 안정 후 숄더 프레스/풀다운/로우 패턴으로 연결',
    };
  }
  ...
  if (/전거근/.test(target)) {
    return {
      role,
      purpose: ['견갑 전인 안정', '갈비-견갑 연결', '오버헤드 안정성 보조'],
      points: ['갈비가 들리지 않게 복압을 잡는다', '날개뼈가 등 위에서 부드럽게 미끄러지게 한다', '어깨 앞쪽으로 찌르지 않고 겨드랑이 아래 힘을 유지', '목 긴장 없이 팔을 길게 보낸다'],
      keywords: ['겨드랑이 아래 단단함', '날개뼈가 붙어 움직이는 느낌', '어깨 앞 부담 감소'],
      muscles: ['전거근', '하부 승모근 보조'],
      signals: ['날개뼈 들뜸', '갈비 들림', '목/상부 승모 과긴장'],
      next: '전거근 활성 후 숄더 프레스/푸쉬업/오버헤드 패턴으로 연결',
    };
  }
